...
test('blog index and tag navigation render without browser errors', async ({ page }) => {
  ...
  const articleLink = page.locator('a.blog-article-row').filter({ hasText: 'How I reached 100 in Lighthouse' });
...
test('article renders semantic heading, chronology, and no hydration errors', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1 })).toHaveText('How I reached 100 in Lighthouse');
  ...
  await page.locator('#what-changed').evaluate((element) => element.scrollIntoView({ block: 'center' }));
