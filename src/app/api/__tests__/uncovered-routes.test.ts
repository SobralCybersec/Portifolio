...
jest.mock('@/lib/ratelimit', () => ({ chatLimit: { limit: jest.fn() } }));
...
const limit = jest.requireMock('@/lib/ratelimit').chatLimit.limit as jest.Mock;
const redis = jest.requireMock('@/lib/chat').redis as Record<string, jest.Mock>;
const pusher = jest.requireMock('@/lib/chat').pusher as { trigger: jest.Mock };
