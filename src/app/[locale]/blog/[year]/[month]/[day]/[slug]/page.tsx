...
import { BlogChrome, BlogChronology, BlogDate, BlogBackLink, BlogTag } from '@/components/blog/BlogUI';
...
export default async function BlogPostPage({ params }: { params: Promise<Params> }) {
  ...
  return <BlogChrome><article className="blog-post-wrapper"><div className="blog-post-inner"><div className="blog-post-back-row"><BlogBackLink locale={locale} /></div><header className="blog-post-header"><div className="blog-post-meta-row"><BlogDate post={post} />{post.draft && <span className="blog-post-category">DRAFT</span>}</div><h1 className="blog-post-title">{post.title}</h1><p className="blog-post-description">{post.description}</p><div className="blog-post-tags" aria-label="Article tags">{tags.map((tag) => <BlogTag className="blog-post-category" key={tag} label={tag} />)}</div>{post.cover && <div className="blog-post-cover"><Image src={post.cover} alt={post.title} fill sizes="(max-width: 800px) 100vw, 800px" priority /></div>}</header><div className="blog-post-layout"><BlogToc items={toc} /><div className="blog-post-body"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: jsonLd }} /><div className="blog-post-content"><BlogMdx source={body} /></div></div></div><BlogChronology current={post} previous={previous} next={next} locale={locale} /></div></article></BlogChrome>;
}
