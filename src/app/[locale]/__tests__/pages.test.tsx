...
jest.mock('@/hooks/useClickSound', () => ({ useClickSound: jest.fn() }));
jest.mock('@/components/Navigation', () => {
  function NavigationMock() { return <nav data-testid="navigation" />; }
  return NavigationMock;
});
jest.mock('@/components/ScrollEffect', () => {
...
jest.mock('@/components/SoloLevelingProjectCard', () => {
...
test('loads projects, searches, filters, and handles empty API response', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: 'TypeScript TypeScript' }));
  expect(screen.getByText('demo-project')).toBeInTheDocument();
  fireEvent.click(screen.getAllByRole('button', { name: 'TypeScript TypeScript' })[0]);
  ...
  expect(screen.getByText('demo-project')).toBeInTheDocument();
  fireEvent.click(screen.getAllByRole('button', { name: 'React React' })[0]);
...
test('covers project language, tech, description filters, and light loading state', async () => {
  ...
  fireEvent.click(screen.getAllByRole('button', { name: 'All Projects' })[0]);
  fireEvent.click(screen.getByRole('button', { name: 'JavaScript JavaScript' }));
  fireEvent.click(screen.getAllByRole('button', { name: 'All Technologies' })[0]);
  fireEvent.click(screen.getByRole('button', { name: 'Next.js Next.js' }));
  const search = screen.getByPlaceholderText('Search projects...');
