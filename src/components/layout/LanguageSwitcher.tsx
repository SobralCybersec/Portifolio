...
export default function LanguageSwitcher() {
  ...
  const handleLanguageChange = (newLocale: string) => {
    ...
    const hasLocalePrefix = Boolean(segments[1] && languages.some((lang) => lang.code === segments[1]));
    const routePath = hasLocalePrefix ? `/${segments.slice(2).join('/')}` : pathname || '/';
    const normalizedRoute = routePath === '/' || routePath === '' ? '/' : routePath.replace(/\/+$/, '');
    const newPath = `/${newLocale}${normalizedRoute === '/' ? '' : normalizedRoute}`;
    const commitLocaleChange = () => router.push(newPath);
    const handled = pageTransition.navigate(normalizedRoute, { commit: commitLocaleChange });
    if (!handled && !pageTransition.isTransitioning) commitLocaleChange();
    setIsOpen(false);
  };
